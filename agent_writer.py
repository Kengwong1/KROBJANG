...
def เขียนบทความ(หัวข้อ: str, หมวด: str) -> str:
    ...
    for i, sec in enumerate(sections):
        ...
        if is_story:
            # Story mode: prompt ต่อเนื่องเรื่องราว ไม่ใช่บทความ
            _story_sec_styles = {
                "story":         "เล่านิทานต่อจากตอนก่อน บรรยากาศสนุก ตัวละครมีชีวิตชีวา",
                "ghost":         "เล่าเรื่องสยองขวัญต่อ ค่อยๆ สร้างความตึงเครียด หักมุมเล็กน้อย",
                "comedy":        "เล่าต่อแบบตลก มีมุกแซว บทสนทนาสนุก",
                "folktale":      "เล่าเรื่องพื้นบ้านต่อ สไตล์โบราณ มีศีลธรรมแฝง",
                "cartoon":       "เล่าการผจญภัยของตัวละครต่อ สีสัน น่ารัก สนุกสนาน",
                "drama":         "เล่าฉากดราม่าต่อ อารมณ์เข้มข้น ความขัดแย้ง ความรู้สึก",
                "inspirational": "เล่าเรื่องสร้างแรงบันดาลใจต่อ อบอุ่น มีพลัง",
            }
            _style = _story_sec_styles.get(หมวด, "เล่าเรื่องต่อ ภาษาสวยงาม น่าติดตาม")
            section_raw = เรียก_ollama(
                f"เรื่อง \"{หัวข้อ}\" ตอน: \"{sec['h']}\"\n"
                f"สไตล์: {_style}\n"
                f"กฎ:\n"
                f"- เขียนเนื้อเรื่องต่อเนื่อง 4-6 ย่อหน้า รวม 400-600 คำ\n"
                f"- มีบทสนทนา (dialog) อย่างน้อย 1 ครั้ง\n"
                f"- บรรยายฉาก สถานที่ อารมณ์ให้เห็นภาพ\n"
                f"- ปิดตอนด้วยประโยคที่ทำให้อยากอ่านตอนต่อไป\n"
                f"- เขียน Markdown เท่านั้น ห้าม HTML",
        ...
        # แทรกรูป: story mode ทุก 2 section, ปกติแค่ section กลาง
        mid = len(sections) // 2
        should_insert_img = (
            (is_story and i > 0 and i % _story_img_every == 0) or
            (not is_story and i == mid)
        )
        if should_insert_img:
            import hashlib as _hlib
            seed = _hlib.md5(หัวข้อ.encode()).hexdigest()[:12]
            fallback = f"https://picsum.photos/seed/{seed}/800/350"
            # FIX: story mode ใช้ keyword คงที่เสมอ (ไม่ให้ AI คิดเอง เพราะมักผิดพลาด)
            if is_story:
                _img_kw = story_img_kw or "colorful illustration storybook children"
                img_url = ดึงรูป_ตรงเนื้อหา(_img_kw, หมวด, _img_kw)
                img_html = (
                    f'<figure style="margin:1.5rem 0;border-radius:12px;overflow:hidden;'
                    f'box-shadow:0 2px 12px rgba(0,0,0,0.15);">'
                    f'<img src="{img_url}" alt="{หัวข้อ}" loading="lazy" '
                    f'onerror="this.onerror=null;this.src=\'{fallback}\'" '
                    f'style="width:100%;max-height:350px;object-fit:cover;">'
                    f'<figcaption style="text-align:center;font-size:0.85rem;color:#64748b;'
                    f'padding:0.5rem;">{หัวข้อ}</figcaption></figure>'
                )
            else:
